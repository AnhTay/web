...
require $_SERVER['DOCUMENT_ROOT'].'/TUANORI_SYSTEM/ketnoi@data.php';
...
if($user['username'] == 'knvnpro'){
    exit('');
}
if($user['username'] == 'kunkeypr'){
    exit('');
}
        if($user['admin'] != 100) {
            if($user['username']) {
            ...
            $path = $root.'/upload/access/log_access.txt';
            $f = fopen($path, 'a');
    ...
    public function parse_access_url() {
        ...
        $user = $this->user();
            ...
            foreach ($revert as $key) {
                $khaideveloper = explode("|", $key);

                    $data[$i] = array(
                        'username' => $khaideveloper[0],
                        'url' => $khaideveloper[1],
                        'ip' => $khaideveloper[3],
                        'time' => $khaideveloper[2]
                    );
    ...
    public function settings($option) {
        ...
        $_key = mysqli_fetch_assoc(mysqli_query($this->connect_db(), "SELECT * FROM `settings` WHERE `key`='web_napthe'"));
        $_kimcuong = mysqli_fetch_assoc(mysqli_query($this->connect_db(), "SELECT * FROM `settings` WHERE `key`='web_kimcuong'"));
        $_chietkhau = mysqli_fetch_assoc(mysqli_query($this->connect_db(), "SELECT * FROM `settings` WHERE `key`='web_chietkhau'"));
        $_mohom = mysqli_fetch_assoc(mysqli_query($this->connect_db(), "SELECT * FROM `settings` WHERE `key`='web_mohom'"));
        ...
        switch ($option) {
            ...
            case 'brick':
                ...
                break;
             case 'kimcuong':
                    return json_decode($_kimcuong['value'], true);
                    break;
            case 'chietkhau':
                    return json_decode($_chietkhau['value'], true);
                    break;
            case 'mohom':
                    return json_decode($_mohom['value'], true);
                    break;
...
return $id;
    ...
    public function login_required() {
        if(!$_SESSION['token']) {
               echo '<script>location.href="/dang-nhap";</script>';
        }
    }
   ...
   public function check_user_fb_register($idfb) {
    ...
    public function is_congtacvien() {
        $user = $this->user();
        $rule = $user['admin'];
        if($rule == 19) {    
            return true;
    ...
    public function userxem() {
        $userxem = $_SESSION['userxem'];
        $result = mysqli_query($this->connect_db(), "SELECT * FROM `users` WHERE `token`='".$token."'");
        $row = mysqli_fetch_array($result,MYSQLI_ASSOC);
        return $row;
    }
   ...
   public function check_token_api($token) {
    ...
    public function vongquaykimcuong_image($id, $type) {
        ...
        $path = $_SERVER["DOCUMENT_ROOT"]."/upload/wheel_data/".$type;
                  ...
                  if ($opendirectory = opendir($path)){
                    while (($file = readdir($opendirectory)) !== false){
                        ...
                        if ($file != "." && $file != "..") {
                            $arr = explode(".", $file);
                            if($arr[0] == $id) {
                               return '/upload/wheel_data/'.$type.'/'.$file;
                            }
                        }
                    }
